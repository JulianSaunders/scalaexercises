...
object Bill2 {
  ...
  def calcBill(order: List[String]): BigDecimal = {
    ...
    (amount + serviceCharge(items, amount)).setScale(2, RoundingMode.HALF_EVEN)
  }

  private def serviceCharge(items: List[Item], amount: BigDecimal): BigDecimal = {
    items.map(_.typ match {
